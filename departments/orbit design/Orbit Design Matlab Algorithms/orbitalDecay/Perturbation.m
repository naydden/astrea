function pert = Perturbation( a,e,i,Bc )
%% Perturbations
...
%J2 pertubation________________ PER REVOLUCIÓ
j=1;
pert(j,5)=-1.5*n*J2*(RE/a)^2*cos(i)*(1-e^2)^-2;
pert(j,4)=0.75*n*J2*(RE/a)^2*(4-5*(sin(i))^2)*(1-e^2)^-2;

%Atmospheric Drag______________ PER REVOLUCIÓ
...
%PREGUNTES PER @tortinez
%- Què són incP, incv
%- De on surten les constants de MOON