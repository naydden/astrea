...
%lamda: is the latitude of the place in degrees[-90,90]
%long: is the longitude of the place iin degrees [0,360)
...
Rg=cos(lambda); 
%distance between the Earth point to the rotation axis of the Earth [0,1]
hg=sin(lambda); 
%distance between th Earth point to the equatorial plane [0,1]
%this 2 distances are measured in earth radius. 

w=2*pi/(24*60); %angular velocity of the Earth [rad/min]  

X=[Rg*cos(w*t+long);Rg*sin(w*t+long);hg*ones(1,length(t))]; 
%position of the point in X_I
%for the given time. The point descrives a cicumference 
%with a Rg radius in the horizontal
